...
function apdl_prepare_input_file_DoublyCoatedDisc(discobj)
    ...
    % IDENTIFY COATING BODIES AND SUBSTRATE BODY

    body_list = [1,2,3];
    coat_list = [];
    for jj_body = 1:3
        SearchString = ['sectype,',num2str(jj_body),',shell'];
        line_indexes = text_find(InputFileEdited, SearchString);
        if ~isempty(line_indexes)
            coat_list(end+1) = jj_body;
        end
    end
    subs_list = setxor(body_list,intersect(body_list,coat_list));

    % STORE ELEMENT NUMBERS IN THE OUTPUT FILE

    SearchString = '*get,_numelem,elem,0,count';
    ReplaceString = ['*get,_numelem,elem,0,count',newline,...
        'esel,s,TYPE,,',num2str(subs_list(1)),newline,...
        '*get,myelsubs,elem,0,count',newline,...
        'esel,s,TYPE,,',num2str(coat_list(1)),newline,...
        '*get,myelcoat1,elem,0,count',newline,...
        'esel,s,TYPE,,',num2str(coat_list(2)),newline,...
        '*get,myelcoat2,elem,0,count',newline,...
        'esel,all',newline,...
        '/com,--- Number of total elements subs = %myelsubs%',newline,...
        '/com,--- Number of total elements coat_one = %myelcoat1%',newline,...
        '/com,--- Number of total elements coat_two = %myelcoat2%'];

    text_replace(InputFileEdited, InputFileEdited, SearchString, ReplaceString);

    % EDIT DENSITY, YOUNG, POISSON, THICKNESS 

    SearchStringList = {};
    ReplaceStringList = {};

    for jj_body = subs_list
    ...
    for jj_body = coat_list
        density = discobj.coating.material.density;
        SearchStringList{end+1} = ['MP,DENS,',num2str(jj_body),','];
        ReplaceStringList{end+1} = ['MP,DENS,',num2str(jj_body),',',num2str(density),', ! kg m^-3   - EDITED BY MATLAB'];
        % if material is amorphous (isotropic)
        if strcmp(discobj.coating.material.flag_anisotropic,'amorph') 
            if ~isempty(discobj.coating.material.young_amorph)
                young = discobj.coating.material.young_amorph;
            else
                young = discobj.coating.material.young_poly;
            end
            SearchStringList{end+1} = ['MP,EX,',num2str(jj_body),','];
            ReplaceStringList{end+1} = ['MP,EX,',num2str(jj_body),',',num2str(young),', ! Pa   - EDITED BY MATLAB'];
            if ~isempty(discobj.coating.material.poisson_amorph)
                poisson = discobj.coating.material.poisson_amorph;
            else
                poisson = discobj.coating.material.poisson_poly;
            end    
            SearchStringList{end+1} = ['MP,NUXY,',num2str(jj_body),','];
            ReplaceStringList{end+1} = ['MP,NUXY,',num2str(jj_body),',',num2str(poisson),', ! - EDITED BY MATLAB'];    
        else
            % if material is polycrystalline (isotropic)    
            if strcmp(discobj.coating.material.flag_anisotropic,'poly') 
                if ~isempty(discobj.coating.material.young_poly) 
                    young = discobj.coating.material.young_poly;
                else
                    young = discobj.coating.material.young_poly_from_stiffness_matrix();
                end
                SearchStringList{end+1} = ['MP,EX,',num2str(jj_body),','];
                ReplaceStringList{end+1} = ['MP,EX,',num2str(jj_body),',',num2str(young),', ! Pa   - EDITED BY MATLAB'];       
                if ~isempty(discobj.coating.material.poisson_poly) 
                    poisson = discobj.coating.material.poisson_poly;
                else
                    poisson = discobj.coating.material.poisson_poly_from_stiffness_matrix();
                end        
                SearchStringList{end+1} = ['MP,NUXY,',num2str(jj_body),','];
                ReplaceStringList{end+1} = ['MP,NUXY,',num2str(jj_body),',',num2str(poisson),', ! - EDITED BY MATLAB'];  
            else
                % if material is single crystal (anisotropic)    
                if strcmp(discobj.coating.material.flag_anisotropic,'single_crystal')  
                    if ~isempty(discobj.coating.material.stiffness_matrix)
                        stiffness_matrix = discobj.coating.material.stiffness_matrix;
                        SearchStringList{end+1} = ['MP,EX,',num2str(jj_body),','];
                        SearchStringList{end+1} = ['MP,NUXY,',num2str(jj_body),','];
                        ReplaceStringList{end+1} = '';
                        % Ansys uses an unusual order in Voigt's vector components:
                        ...
                        young = mean(cell2mat(struct2cell(discobj.coating.material.young_directional)));
                        SearchStringList{end+1} = ['MP,EX,',num2str(jj_body),','];
                        ReplaceStringList{end+1} = ['MP,EX,',num2str(jj_body),',',num2str(young),', ! Pa   - EDITED BY MATLAB'];                
                        poisson = mean(cell2mat(struct2cell(discobj.coating.material.poisson_directional)));
                        SearchStringList{end+1} = ['MP,NUXY,',num2str(jj_body),','];
                        ReplaceStringList{end+1} = ['MP,NUXY,',num2str(jj_body),',',num2str(poisson),', ! - EDITED BY MATLAB']; 
                    end
                end
    ...
    for jj_body = coat_list
        line_index = [];
        SearchString = ['sectype,',num2str(jj_body),',shell'];
        line_index = text_find(InputFileEdited, SearchString);
        thickness = discobj.coating.thickness;
        ReplaceString = ['secdata,',num2str(thickness,'%1.1E'),' ! - EDITED BY MATLAB'];
        line_replace(InputFileEdited, InputFileEdited, line_index+1, ReplaceString);
    end    

    % Compact element numbers sequentially: this is necessary with coatings 
    % because any gap in the element numbering (as is the case b/w solid and shell elements) 
    % throws off the array construction in the post-processing script
    AddString = [newline '/gopr' newline 'numcmp,elem' newline];
    line_add(InputFileEdited, InputFileEdited, line_index+2, AddString);
    ...
    % (necessary to keep full integration in solid186 and
    % to avoid keyopt(8)=2 in shell281)
    ...
    %     disp('Warning: element type for substrate does not appear to be SOLID186 - check input_file_edited.dat')
    % end
    ...
    % end
    % line_index2 = text_find(InputFileEdited, 'et,2,281');
    % line_index3 = text_find(InputFileEdited, 'et,3,281');
    % if isempty(line_index2) || isempty(line_index3)
    %     disp('Warning: element type for coating does not appear to be SHELL281 - check input_file_edited.dat')
    % end
